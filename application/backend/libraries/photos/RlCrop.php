...
class RlCrop {
    ...
    { 	
        ...
        $photo_info = $this->ci->db->query("SELECT * FROM ".PREFIX."photos WHERE id = '{$id}'")->row();
		...
		$thumbnail_file = ROOTPATH . $thumbnail_name;
		$thumbnail_url = site_url() . $thumbnail_name;
		
		$photo_name = $dir_name . '/large_'. time() . mt_rand(). '.' .$ext;
		$photo_file = ROOTPATH . $photo_name;
		
		$this->loadImage(ROOTPATH . $photo_info->original );
		$this->cropToDimensions(ceil($coords[0]), ceil($coords[1]), ceil($coords[2]), ceil($coords[3]));
		$this->saveImage($photo_file, 100); // 100 image quality confige tanımla
		$this->flushImages();

		if ( is_readable($photo_file) )
		{
			$this->ci->rlresize->resize($photo_file, $thumbnail_file, 'C', array(150, 120), null, false);
			
			$size = getimagesize($photo_file); // onur
			$sizeArray = $size[0] > $size[1] ? array(640, 480) : array(480, 640); // onur
			$this->ci->rlresize->resize($photo_file, $photo_file, 'C', $sizeArray, null, true);
		}
		
		if ( is_readable($photo_file) && is_readable($thumbnail_file) )
		{
			unlink( ROOTPATH . $photo_info->photo );
			unlink( ROOTPATH . $photo_info->thumbnail );
			...
			$this->ci->db->update(PREFIX.'photos', $data);
		}

		echo json_encode(array('thumbnail' => $thumbnail_url, 'text' => "İşlem başarılı"));
    }
}