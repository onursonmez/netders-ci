...
<div class="panel-body">
  ...
  <form method="post" action="<?=base_url('backend/lessons')?><?if(strstr(uri_string(), 'addcategory') == TRUE):?>/addcategory<?else:?>/editcategory/<?=$this->uri->segment(4)?><?endif;?>" class="form-horizontal" onsubmit="return prepareSubmit(this);">
  <div class="tab-content">
    ...
    <div class="tab-pane<?if($lang->lang_code == DESCR_SL):?> active<?endif;?>" id="lang-<?=$lang->lang_code?>">
		...
		<div class="clear m-b">
			<a class="btn btn-default btn-xs pull-right" onclick="confirmation('Lütfen dikkat!', 'Kategori &quot;<?=$lang->name?>&quot; dilinde kalıcı olarak silinecektir', '<?=base_url('backend/lessons/deletecategorylang/'.$this->uri->segment(4).'/'.$lang->lang_code)?>'); return false;" href="#"><i class="fa fa-trash-o"></i> <strong><?=$lang->name?></strong> dilde kategoriyi sil</a>
		</div>
		<?endif;?>		
