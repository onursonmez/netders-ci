...
<section class="panel panel-default">
	<header class="panel-heading">
		...
		<span class="pull-right"><a href="<?=base_url('backend/sliders/add')?>"><i class="fa fa-plus"></i> <?=lang('SLIDERS_NEW')?></a></span>
	</header>
		...
		<table class="table table-striped m-b-none" data-ride="datatables">
			<thead>
				<tr>
					...
					<th><?=lang('TITLE')?></th>
					<th>Aktif Bölümler</th>
					<th width="126"><?=lang('OPERATIONS')?></th>
				</tr>
			</thead>
			<tbody>
			</tbody>
	...
	$(function(){
		...
		$('[data-ride="datatables"]').each(function() {
			var oTable = $(this).dataTable({
				"bProcessing": true,
				"bServerSide": true,
				"sAjaxSource": base_url+"backend/sliders",
				...
				"aoColumns": [
					...
					{ "mData": "id",
						...
						{
							var out = '';
							out += '<a href="<?=base_url('backend/sliders/edit')?>/'+val.id+'" title="<?=lang('EDIT')?>" data-toggle="tooltip" class="btn btn-xs btn-default"><i class="fa fa-pencil"></i></a> ';
			            	out += '<a href="<?=base_url('backend/sliders/images')?>/'+val.id+'" title="<?=lang('IMAGES')?>" data-toggle="tooltip" class="btn btn-xs btn-default m-r-xs"><i class="fa fa-picture-o"></i></a>';
							if(val.defaults != 'Y'){
			            		out += '<a onclick="confirmation(\'<?=lang('WARNING')?>\', \'Slayt ve bağlı görseller çöp kutusuna gönderilecektir\', \'<?=base_url('backend/sliders/delete')?>/'+val.id+'\')" class="btn btn-xs btn-default" data-toggle="tooltip" title="<?=lang('DELETE')?>"><i class="fa fa-trash-o"></i></a>';
			            	}			            	
							return out;
