...
class Users extends CI_Controller {
   ...
   public function __construct(){
	...
	public function prices_text($id = null)
	{
		...
		$data['price'] = $this->users_model->get_user_price($id);
	...
	public function view($id = null)
	{
		...
		$data['user'] = $this->users_model->get_user_data((int)$id);		
	...
	public function view_price($uid = null, $id = null)
	{
		$data = array();

		$data['user'] = $this->users_model->get_user_by(array('id' => (int)$uid));		
	...
	public function sendmessage()
	{
		...
		if($this->input->post('form') == 'ajax_message')
		{
			...
			if ($this->form_validation->run() == FALSE){
				...
				if(!empty($from_user) && !empty($from_user->mobile) && $from_user->mobile != $this->input->post('mobile')){
	...
	function captcha()
	{
		echo json_encode(array('image' => '<img src="'.base_url('captcha/'.generate_captcha($this->input->post('form', true))).'" width="100%" height="32" />', 'csrf_name' => $this->security->get_csrf_token_name(), 'csrf_hash' => $this->security->get_csrf_hash()));
		exit;
	}
