...
class Users_model extends CI_Model {
	...
	public function get_user_prices($data = [])
  {
	...
	public function get_user_price($where = [])
  {
    if(empty($where)) return false;

    foreach($where as $key => $value)
    $where[$key] = $value ? $this->security->xss_clean($value) : NULL;
	...
	public function get_user_month($data = array())
	{
		...
		if(!empty($user))
		return $this->get_user_data(['id' => $user->uid]);
	}
