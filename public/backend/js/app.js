...
Modernizr.addTest('firefoxx',function(){return!!navigator.userAgent.match(/Firefoxx/i)});
...
$(window).load(function() {
  // .w-f height
  if($( window ).width() > 768){  
  	$('.w-f').height($('.vbox').outerHeight() - 110);
  }
});
...
+function ($) {
  ...
  $(function(){
	...
	// datetimepicker
    $('.datetimepicker').datetimepicker({locale: 'tr', useCurrent: true, stepping: 5,
    	icons: {
	        time: "fa fa-clock-o",
	        date: "fa fa-calendar",
	        up: "fa fa-arrow-up",
	...
	$.ui.autocomplete.prototype._renderItem = function( ul, item){
	  ...
	  var t = item.label.replace(re,"<strong style=\"color:#333;\">$1</strong>");
	  return $( "<li></li>" )
	...
	if(location.hash) {
		$('a[href=' + location.hash + ']').tab('show').parent('li').addClass('active').scrollTop(0);
	}
	
	if($.cookie('nav-xs') == 'yes'){
		$('#nav').addClass('nav-xs');
		$('.btn-nav-action .i-circleright').removeClass('text-active').addClass('text');
		$('.btn-nav-action .i-circleleft').addClass('text-active');		
	}
	
	$('.btn-nav-action').click(function (e) {
...
function checkImage(form_class){
	if($('.preview').length){
		$.growl('İçerik için fotoğraf seçtiniz ancak YÜKLE butonuna basmadınız!', {type	: 'danger'});
	} else {
...
function confirmation(title, body, url){
	$('#modal-confirmation-id .modal-title').html(title);
	$('#modal-confirmation-id .modal-body').html(body);
	$('#modal-confirmation-id .modal-url').attr('href', url);
	$('#modal-confirmation-id').modal('show');
}
