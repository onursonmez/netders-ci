...
(function (window, document) {
	...
	Croppic = function (photoId, divId, options) {

		var that = this;
		that.photoId = photoId;
		that.divId = divId;
		that.obj = $('#' + divId);
	...
	Croppic.prototype = {
		...
		init: function () {
			...
			that.imgInitW = that.imgW = that.options.originalWidth;
			that.imgInitH = that.imgH = that.options.originalHeight;
			
			if(that.options.modal){	that.createModal(); }
			
			if( !$.isEmptyObject(that.croppedImg)){ that.croppedImg.remove(); }
			
			that.obj.append('<img src="'+that.options.imageUrl+'">');
		...
		bindImgUploadControl: function(){
			...
			var formHtml = '<form class="'+that.divId+'_imgUploadForm" style="display: none; visibility: hidden;">  <input type="file" name="img">  </form>';
			that.outputDiv.append(formHtml);
			that.form = that.outputDiv.find('.'+that.divId+'_imgUploadForm');
			...
			that.form.find('input[type="file"]').change(function(){
				...
				for (var key in that.options.extraData) {
					if( that.options.extraData.hasOwnProperty(key) ) {
						formData.append( key , that.options.extraData[key] );	
					}
				}
		...
		*/
		...
		createCropControls: function(){
			...
			that.cropControlReset.on('click',function(){ that.destroy(); });				
			...
			$.ajax({
                ...
                type: 'POST'
					...
					if(response.status=='success'){
						...
						if(that.options.imageUpdate)
						$('.thumb-'+that.photoId).attr({src: response.url+'?' + new Date().getTime()});
						
						$.growl('İşleminiz başarıyla tamamlanmıştır');
						
						that.hideLoader();

					}
					if(response.status=='error'){
						$.growl(response.message, {type	: 'danger'});
					}
